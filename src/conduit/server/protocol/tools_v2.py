...
class ToolManager:
    ...
    def add_client_tool(
        ...
        """Add a tool specific to a client.
    ...
    def get_client_specific_tools(self, client_id: str) -> dict[str, Tool]:
        """Get only the client-specific tools for a client (excludes global).

        Useful for debugging, introspection, or when you need to see exactly
        which tools are registered specifically for a client.

        Args:
            client_id: ID of the client to get specific tools for.

        Returns:
            Dictionary of client-specific tools, empty if none registered.
        """
        if client_id not in self.client_tools:
            return {}
